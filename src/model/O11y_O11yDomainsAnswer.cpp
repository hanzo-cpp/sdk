/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator's admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.14.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */



#include "hanzo/model/O11y_O11yDomainsAnswer.h"

namespace hanzo {
namespace model {

O11y_O11yDomainsAnswer::O11y_O11yDomainsAnswer()
{
    m_DataIsSet = false;
    m_MetaIsSet = false;
    m_Type = utility::conversions::to_string_t("");
    m_TypeIsSet = false;
    m_WarningIsSet = false;
}

O11y_O11yDomainsAnswer::~O11y_O11yDomainsAnswer()
{
}

void O11y_O11yDomainsAnswer::validate()
{
    // TODO: implement validation
}

web::json::value O11y_O11yDomainsAnswer::toJson() const
{
    web::json::value val = web::json::value::object();
    if(m_DataIsSet)
    {
        
        val[utility::conversions::to_string_t(_XPLATSTR("data"))] = ModelBase::toJson(m_Data);
    }
    if(m_MetaIsSet)
    {
        
        val[utility::conversions::to_string_t(_XPLATSTR("meta"))] = ModelBase::toJson(m_Meta);
    }
    if(m_TypeIsSet)
    {
        
        val[utility::conversions::to_string_t(_XPLATSTR("type"))] = ModelBase::toJson(m_Type);
    }
    if(m_WarningIsSet)
    {
        
        val[utility::conversions::to_string_t(_XPLATSTR("warning"))] = ModelBase::toJson(m_Warning);
    }

    return val;
}

bool O11y_O11yDomainsAnswer::fromJson(const web::json::value& val)
{
    bool ok = true;
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("data"))))
    {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t(_XPLATSTR("data")));
        if(!fieldValue.is_null())
        {
            std::shared_ptr<O11y_O11yDomainsData> refVal_setData;
            ok &= ModelBase::fromJson(fieldValue, refVal_setData);
            setData(refVal_setData);
            
        }
    }
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("meta"))))
    {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t(_XPLATSTR("meta")));
        if(!fieldValue.is_null())
        {
            std::shared_ptr<O11y_O11yQueryStats> refVal_setMeta;
            ok &= ModelBase::fromJson(fieldValue, refVal_setMeta);
            setMeta(refVal_setMeta);
            
        }
    }
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("type"))))
    {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t(_XPLATSTR("type")));
        if(!fieldValue.is_null())
        {
            utility::string_t refVal_setType;
            ok &= ModelBase::fromJson(fieldValue, refVal_setType);
            setType(refVal_setType);
            
        }
    }
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("warning"))))
    {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t(_XPLATSTR("warning")));
        if(!fieldValue.is_null())
        {
            std::shared_ptr<O11y_O11yQueryWarning> refVal_setWarning;
            ok &= ModelBase::fromJson(fieldValue, refVal_setWarning);
            setWarning(refVal_setWarning);
            
        }
    }
    return ok;
}

void O11y_O11yDomainsAnswer::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
    {
        namePrefix += utility::conversions::to_string_t(_XPLATSTR("."));
    }
    if(m_DataIsSet)
    {
        multipart->add(ModelBase::toHttpContent(namePrefix + utility::conversions::to_string_t(_XPLATSTR("data")), m_Data));
    }
    if(m_MetaIsSet)
    {
        multipart->add(ModelBase::toHttpContent(namePrefix + utility::conversions::to_string_t(_XPLATSTR("meta")), m_Meta));
    }
    if(m_TypeIsSet)
    {
        multipart->add(ModelBase::toHttpContent(namePrefix + utility::conversions::to_string_t(_XPLATSTR("type")), m_Type));
    }
    if(m_WarningIsSet)
    {
        multipart->add(ModelBase::toHttpContent(namePrefix + utility::conversions::to_string_t(_XPLATSTR("warning")), m_Warning));
    }
}

bool O11y_O11yDomainsAnswer::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
    {
        namePrefix += utility::conversions::to_string_t(_XPLATSTR("."));
    }

    if(multipart->hasContent(utility::conversions::to_string_t(_XPLATSTR("data"))))
    {
        std::shared_ptr<O11y_O11yDomainsData> refVal_setData;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(_XPLATSTR("data"))), refVal_setData );
        setData(refVal_setData);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(_XPLATSTR("meta"))))
    {
        std::shared_ptr<O11y_O11yQueryStats> refVal_setMeta;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(_XPLATSTR("meta"))), refVal_setMeta );
        setMeta(refVal_setMeta);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(_XPLATSTR("type"))))
    {
        utility::string_t refVal_setType;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(_XPLATSTR("type"))), refVal_setType );
        setType(refVal_setType);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(_XPLATSTR("warning"))))
    {
        std::shared_ptr<O11y_O11yQueryWarning> refVal_setWarning;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(_XPLATSTR("warning"))), refVal_setWarning );
        setWarning(refVal_setWarning);
    }
    return ok;
}


std::shared_ptr<O11y_O11yDomainsData> O11y_O11yDomainsAnswer::getData() const
{
    return m_Data;
}


void O11y_O11yDomainsAnswer::setData(const std::shared_ptr<O11y_O11yDomainsData>& value)
{
    m_Data = value;
    m_DataIsSet = true;
}

bool O11y_O11yDomainsAnswer::dataIsSet() const
{
    return m_DataIsSet;
}

void O11y_O11yDomainsAnswer::unsetData()
{
    m_DataIsSet = false;
}
std::shared_ptr<O11y_O11yQueryStats> O11y_O11yDomainsAnswer::getMeta() const
{
    return m_Meta;
}


void O11y_O11yDomainsAnswer::setMeta(const std::shared_ptr<O11y_O11yQueryStats>& value)
{
    m_Meta = value;
    m_MetaIsSet = true;
}

bool O11y_O11yDomainsAnswer::metaIsSet() const
{
    return m_MetaIsSet;
}

void O11y_O11yDomainsAnswer::unsetMeta()
{
    m_MetaIsSet = false;
}
utility::string_t O11y_O11yDomainsAnswer::getType() const
{
    return m_Type;
}


void O11y_O11yDomainsAnswer::setType(const utility::string_t& value)
{
    m_Type = value;
    m_TypeIsSet = true;
}

bool O11y_O11yDomainsAnswer::typeIsSet() const
{
    return m_TypeIsSet;
}

void O11y_O11yDomainsAnswer::unsetType()
{
    m_TypeIsSet = false;
}
std::shared_ptr<O11y_O11yQueryWarning> O11y_O11yDomainsAnswer::getWarning() const
{
    return m_Warning;
}


void O11y_O11yDomainsAnswer::setWarning(const std::shared_ptr<O11y_O11yQueryWarning>& value)
{
    m_Warning = value;
    m_WarningIsSet = true;
}

bool O11y_O11yDomainsAnswer::warningIsSet() const
{
    return m_WarningIsSet;
}

void O11y_O11yDomainsAnswer::unsetWarning()
{
    m_WarningIsSet = false;
}

}
}


