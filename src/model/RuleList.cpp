...
void RuleList::validate()
{
    // TODO: implement validation
}

web::json::value RuleList::toJson() const
{
...
bool RuleList::fromJson(const web::json::value& val)
{
    bool ok = true;
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("data"))))
    {
        ...
        {
            std::vector<std::shared_ptr<RuleView>> refVal_setData;
...
void RuleList::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
bool RuleList::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
...
std::vector<std::shared_ptr<RuleView>> RuleList::getData() const
{
    return m_Data;
}


void RuleList::setData(const std::vector<std::shared_ptr<RuleView>>& value)
{
    m_Data = value;
    m_DataIsSet = true;
}

bool RuleList::dataIsSet() const
{
    return m_DataIsSet;
}

void RuleList::unsetData()
{
    m_DataIsSet = false;
}
