...
bool ItemList::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("data"))))
    {
        ...
        {
            std::vector<std::shared_ptr<ItemView>> refVal_setData;
...
void ItemList::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
bool ItemList::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
...
std::vector<std::shared_ptr<ItemView>> ItemList::getData() const
{
    return m_Data;
}


void ItemList::setData(const std::vector<std::shared_ptr<ItemView>>& value)
{
    m_Data = value;
    m_DataIsSet = true;
}

bool ItemList::dataIsSet() const
{
    return m_DataIsSet;
}

void ItemList::unsetData()
{
    m_DataIsSet = false;
}
