...
void StepList::validate()
{
    // TODO: implement validation
}

web::json::value StepList::toJson() const
{
...
bool StepList::fromJson(const web::json::value& val)
{
    bool ok = true;
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("data"))))
    {
        ...
        {
            std::vector<std::shared_ptr<Step>> refVal_setData;
...
void StepList::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
bool StepList::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
...
std::vector<std::shared_ptr<Step>> StepList::getData() const
{
    return m_Data;
}


void StepList::setData(const std::vector<std::shared_ptr<Step>>& value)
{
    m_Data = value;
    m_DataIsSet = true;
}

bool StepList::dataIsSet() const
{
    return m_DataIsSet;
}

void StepList::unsetData()
{
    m_DataIsSet = false;
}
