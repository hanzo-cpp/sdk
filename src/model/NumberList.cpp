...
void NumberList::validate()
{
    // TODO: implement validation
}

web::json::value NumberList::toJson() const
{
...
bool NumberList::fromJson(const web::json::value& val)
{
    bool ok = true;
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("data"))))
    {
        ...
        {
            std::vector<std::shared_ptr<Number>> refVal_setData;
...
void NumberList::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
bool NumberList::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
...
std::vector<std::shared_ptr<Number>> NumberList::getData() const
{
    return m_Data;
}


void NumberList::setData(const std::vector<std::shared_ptr<Number>>& value)
{
    m_Data = value;
    m_DataIsSet = true;
}

bool NumberList::dataIsSet() const
{
    return m_DataIsSet;
}

void NumberList::unsetData()
{
    m_DataIsSet = false;
}
