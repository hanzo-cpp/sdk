...
bool Runs::fromJson(const web::json::value& val)
{
    bool ok = true;
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("data"))))
    {
        ...
        {
            std::vector<std::shared_ptr<RunRecord>> refVal_setData;
...
bool Runs::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
...
std::vector<std::shared_ptr<RunRecord>> Runs::getData() const
{
    return m_Data;
}


void Runs::setData(const std::vector<std::shared_ptr<RunRecord>>& value)
{
    m_Data = value;
    m_DataIsSet = true;
}

bool Runs::dataIsSet() const
{
    return m_DataIsSet;
}
