...
web::json::value EventList::toJson() const
{
...
bool EventList::fromJson(const web::json::value& val)
{
    bool ok = true;
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("data"))))
    {
        ...
        {
            std::vector<std::shared_ptr<ProductEvent>> refVal_setData;
...
void EventList::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
bool EventList::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
...
std::vector<std::shared_ptr<ProductEvent>> EventList::getData() const
{
    return m_Data;
}


void EventList::setData(const std::vector<std::shared_ptr<ProductEvent>>& value)
{
    m_Data = value;
    m_DataIsSet = true;
}

bool EventList::dataIsSet() const
{
    return m_DataIsSet;
}

void EventList::unsetData()
{
    m_DataIsSet = false;
}
