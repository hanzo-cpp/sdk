...
bool EndpointList::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("data"))))
    {
        ...
        {
            std::vector<std::shared_ptr<Endpoint>> refVal_setData;
...
void EndpointList::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
bool EndpointList::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
...
std::vector<std::shared_ptr<Endpoint>> EndpointList::getData() const
{
    return m_Data;
}


void EndpointList::setData(const std::vector<std::shared_ptr<Endpoint>>& value)
{
    m_Data = value;
    m_DataIsSet = true;
}

bool EndpointList::dataIsSet() const
{
    return m_DataIsSet;
}

void EndpointList::unsetData()
{
    m_DataIsSet = false;
}
