/**
 * Hanzo Cloud API
 * Composed from each subsystem's own projection of its router, in the fleet's mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.14.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * O11y_O11yMetricStat.h
 *
 * 
 */

#ifndef HANZO_MODEL_O11y_O11yMetricStat_H_
#define HANZO_MODEL_O11y_O11yMetricStat_H_

#include <boost/optional.hpp>

#include "hanzo/ModelBase.h"

#include <cpprest/details/basic_types.h>

namespace hanzo {
namespace model {



class  O11y_O11yMetricStat
    : public ModelBase
{
public:
    O11y_O11yMetricStat();
    virtual ~O11y_O11yMetricStat();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;


    /////////////////////////////////////////////
    /// O11y_O11yMetricStat members


    /// <summary>
    /// Description describes the metric.
    /// </summary>
    utility::string_t getDescription() const;
    bool descriptionIsSet() const;
    void unsetDescription();
    void setDescription(const utility::string_t& value);

    /// <summary>
    /// MetricName is the metric&#39;s name.
    /// </summary>
    utility::string_t getMetricName() const;
    bool metricNameIsSet() const;
    void unsetMetricName();
    void setMetricName(const utility::string_t& value);

    /// <summary>
    /// Samples is how many samples the metric had.
    /// </summary>
    int32_t getSamples() const;
    bool samplesIsSet() const;
    void unsetSamples();
    void setSamples(int32_t value);

    /// <summary>
    /// TimeSeries is how many time series the metric had.
    /// </summary>
    int32_t getTimeseries() const;
    bool timeseriesIsSet() const;
    void unsetTimeseries();
    void setTimeseries(int32_t value);

    /// <summary>
    /// Type is the metric type.
    /// </summary>
    utility::string_t getType() const;
    bool typeIsSet() const;
    void unsetType();
    void setType(const utility::string_t& value);

    /// <summary>
    /// Unit is the metric&#39;s unit.
    /// </summary>
    utility::string_t getUnit() const;
    bool unitIsSet() const;
    void unsetUnit();
    void setUnit(const utility::string_t& value);


protected:
    utility::string_t m_Description;
    bool m_DescriptionIsSet;

    utility::string_t m_MetricName;
    bool m_MetricNameIsSet;

    int32_t m_Samples;
    bool m_SamplesIsSet;

    int32_t m_Timeseries;
    bool m_TimeseriesIsSet;

    utility::string_t m_Type;
    bool m_TypeIsSet;

    utility::string_t m_Unit;
    bool m_UnitIsSet;

};


}
}

#endif /* HANZO_MODEL_O11y_O11yMetricStat_H_ */
