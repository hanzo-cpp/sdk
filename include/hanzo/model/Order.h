/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator's admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.14.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * Order.h
 *
 * 
 */

#ifndef HANZO_MODEL_Order_H_
#define HANZO_MODEL_Order_H_

#include <boost/optional.hpp>

#include "hanzo/ModelBase.h"

#include "hanzo/model/Contacts.h"
#include <cpprest/details/basic_types.h>

namespace hanzo {
namespace model {

class Contacts;


class  Order
    : public ModelBase
{
public:
    Order();
    virtual ~Order();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;


    /////////////////////////////////////////////
    /// Order members


    /// <summary>
    /// Contacts is the WHOIS contact set. Omit it and the registrar uses the reseller account&#39;s default contacts.
    /// </summary>
    std::shared_ptr<Contacts> getContacts() const;
    bool contactsIsSet() const;
    void unsetContacts();
    void setContacts(const std::shared_ptr<Contacts>& value);

    /// <summary>
    /// Domain is the name to buy. It is required.
    /// </summary>
    utility::string_t getDomain() const;
    bool domainIsSet() const;
    void unsetDomain();
    void setDomain(const utility::string_t& value);

    /// <summary>
    /// Years is the term to buy, defaulting to 1.
    /// </summary>
    int32_t getYears() const;
    bool yearsIsSet() const;
    void unsetYears();
    void setYears(int32_t value);


protected:
    std::shared_ptr<Contacts> m_Contacts;
    bool m_ContactsIsSet;

    utility::string_t m_Domain;
    bool m_DomainIsSet;

    int32_t m_Years;
    bool m_YearsIsSet;

};


}
}

#endif /* HANZO_MODEL_Order_H_ */
