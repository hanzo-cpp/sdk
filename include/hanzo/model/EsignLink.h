/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator's admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.14.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * EsignLink.h
 *
 * 
 */

#ifndef HANZO_MODEL_EsignLink_H_
#define HANZO_MODEL_EsignLink_H_

#include <boost/optional.hpp>

#include "hanzo/ModelBase.h"

#include <cpprest/details/basic_types.h>

namespace hanzo {
namespace model {



class  EsignLink
    : public ModelBase
{
public:
    EsignLink();
    virtual ~EsignLink();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;


    /////////////////////////////////////////////
    /// EsignLink members


    /// <summary>
    /// Email is the address this link is meant for.
    /// </summary>
    utility::string_t getEmail() const;
    bool emailIsSet() const;
    void unsetEmail();
    void setEmail(const utility::string_t& value);

    /// <summary>
    /// RecipientID is the recipient the link identifies.
    /// </summary>
    utility::string_t getRecipientId() const;
    bool recipientIdIsSet() const;
    void unsetRecipientId();
    void setRecipientId(const utility::string_t& value);

    /// <summary>
    /// Role is their role — only a SIGNER or an APPROVER gets a link, because only they are asked to act.
    /// </summary>
    utility::string_t getRole() const;
    bool roleIsSet() const;
    void unsetRole();
    void setRole(const utility::string_t& value);

    /// <summary>
    /// SigningPath is the tail of the address to send them, relative to wherever the signing page is served.
    /// </summary>
    utility::string_t getSigningPath() const;
    bool signingPathIsSet() const;
    void unsetSigningPath();
    void setSigningPath(const utility::string_t& value);

    /// <summary>
    /// Token is the crypto-random signing capability. It is the entire credential, so treat it as a secret and give each one only to the recipient it names.
    /// </summary>
    utility::string_t getToken() const;
    bool tokenIsSet() const;
    void unsetToken();
    void setToken(const utility::string_t& value);


protected:
    utility::string_t m_Email;
    bool m_EmailIsSet;

    utility::string_t m_RecipientId;
    bool m_RecipientIdIsSet;

    utility::string_t m_Role;
    bool m_RoleIsSet;

    utility::string_t m_SigningPath;
    bool m_SigningPathIsSet;

    utility::string_t m_Token;
    bool m_TokenIsSet;

};


}
}

#endif /* HANZO_MODEL_EsignLink_H_ */
