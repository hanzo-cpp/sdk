/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator's admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.14.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * InsightsBody.h
 *
 * 
 */

#ifndef HANZO_MODEL_InsightsBody_H_
#define HANZO_MODEL_InsightsBody_H_

#include <boost/optional.hpp>

#include "hanzo/ModelBase.h"

#include "hanzo/AnyType.h"
#include "hanzo/model/InsightsEvent.h"
#include <cpprest/details/basic_types.h>
#include <map>
#include <vector>

namespace hanzo {
namespace model {

class InsightsEvent;


class  InsightsBody
    : public ModelBase
{
public:
    InsightsBody();
    virtual ~InsightsBody();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;


    /////////////////////////////////////////////
    /// InsightsBody members


    std::vector<std::shared_ptr<InsightsEvent>> getBatch() const;
    bool batchIsSet() const;
    void unsetBatch();
    void setBatch(const std::vector<std::shared_ptr<InsightsEvent>>& value);

    utility::string_t getDistinctId() const;
    bool distinctIdIsSet() const;
    void unsetDistinct_id();
    void setDistinctId(const utility::string_t& value);

    utility::string_t getEvent() const;
    bool eventIsSet() const;
    void unsetEvent();
    void setEvent(const utility::string_t& value);

    std::map<utility::string_t, std::shared_ptr<AnyType>> getProperties() const;
    bool propertiesIsSet() const;
    void unsetProperties();
    void setProperties(const std::map<utility::string_t, std::shared_ptr<AnyType>>& value);

    utility::string_t getTimestamp() const;
    bool timestampIsSet() const;
    void unsetTimestamp();
    void setTimestamp(const utility::string_t& value);

    utility::string_t getUuid() const;
    bool uuidIsSet() const;
    void unsetUuid();
    void setUuid(const utility::string_t& value);


protected:
    std::vector<std::shared_ptr<InsightsEvent>> m_Batch;
    bool m_BatchIsSet;

    utility::string_t m_Distinct_id;
    bool m_Distinct_idIsSet;

    utility::string_t m_Event;
    bool m_EventIsSet;

    std::map<utility::string_t, std::shared_ptr<AnyType>> m_Properties;
    bool m_PropertiesIsSet;

    utility::string_t m_Timestamp;
    bool m_TimestampIsSet;

    utility::string_t m_Uuid;
    bool m_UuidIsSet;

};


}
}

#endif /* HANZO_MODEL_InsightsBody_H_ */
