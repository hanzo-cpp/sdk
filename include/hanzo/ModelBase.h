...
#ifndef HANZO_MODEL_ModelBase_H_
#define HANZO_MODEL_ModelBase_H_
...
public:
    ...
    template <typename T, typename std::enable_if<std::is_base_of<ModelBase, T>::value, int>::type = 0>
    static web::json::value toJson( const T& val ) { return val.toJson(); }
    template <typename T, typename std::enable_if<std::is_base_of<ModelBase, T>::value, int>::type = 0>
    static bool fromJson( const web::json::value& val, T& outVal ) { return outVal.fromJson(val); }

    template <typename T, typename std::enable_if<std::is_base_of<ModelBase, T>::value, int>::type = 0>
    static std::shared_ptr<HttpContent> toHttpContent( const utility::string_t& name, const T& val, const utility::string_t& contentType = utility::conversions::to_string_t("application/json") )
    {
        std::shared_ptr<HttpContent> content = std::make_shared<HttpContent>();
        content->setName( name );
        content->setContentDisposition( utility::conversions::to_string_t("form-data") );
        content->setContentType( contentType );
        content->setData( std::make_shared<std::stringstream>( utility::conversions::to_utf8string(val.toJson().serialize()) ) );
        return content;
    }
    template <typename T, typename std::enable_if<std::is_base_of<ModelBase, T>::value, int>::type = 0>
    static bool fromHttpContent( std::shared_ptr<HttpContent> val, T& outVal )
    {
        if (val == nullptr) return false;
        utility::string_t str;
        ModelBase::fromHttpContent(val, str);
        return outVal.fromJson(web::json::value::parse(str));
    }
...
#endif /* HANZO_MODEL_ModelBase_H_ */
